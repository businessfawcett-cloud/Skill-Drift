...
_PROMPT_TEMPLATE = """You are comparing two versions of an AI agent skill. Given the diff below between OLD and NEW versions:
...
1. List concrete differences (added/removed/changed instructions, permissions, tool calls, external references).
2. For each difference, state whether it changes what the skill asks the agent to do, what the skill can access, or what the skill claims to do vs. what it now does.
...
def get_prompt(old_content, new_content):
    ...
    if os.path.isfile(prompt_path):
        with open(prompt_path, "r") as f:
            return f.read().format(old_content=old_content, new_content=new_content)
    return _PROMPT_TEMPLATE.format(old_content=old_content, new_content=new_content)